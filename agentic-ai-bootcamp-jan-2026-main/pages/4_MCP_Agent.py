...
# MCP API Key (optional) - used for servers that require Authorization header (e.g., Zapier MCP)
if "mcp_api_key" not in st.session_state:
    st.session_state.mcp_api_key = ""  # Store MCP API key (can be empty)
...
with st.sidebar:
    ...
    if st.session_state.openai_key or st.session_state.mcp_server_url:
        if st.button("Change API Keys"):
            ...
            st.session_state.mcp_api_key = ""  # Reset MCP API key too
...
if keys_needed:
    ...
    mcp_api_key = st.session_state.mcp_api_key
    ...
    # MCP API key is OPTIONAL (can be empty)
    ...
    if st.button("Connect"):
        ...
        if valid:
            ...
            # Store MCP API key even if empty
...
if not st.session_state.mcp_agent:
    ...
    with st.spinner("Initializing MCP agent..."):
        ...
        try:
            ...
            # Add Authorization header only if MCP API key is provided
            if st.session_state.mcp_api_key:
                server_cfg["headers"] = {
                    "Authorization": f"Bearer {st.session_state.mcp_api_key}"
                }

            mcp_config = {"server": server_cfg}
